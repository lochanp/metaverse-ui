    ...
    <div className={`${styles.innerWidth} mx-auto flex flex-col gap-8`}>
      <div className='flex item-center justify-between flex-wrap gap-5'>
        ...
        <button type='button' className='flex items-center h-fit py-4 px-6 bg-[#25618b] rounded-[32px] gap-[12px]'>
          <img src='/headset.svg' className='w-24px h-24px object-contain'></img>
      ...
      <div className='flex flex-col'>
        ...
        <div className='flex items-center justify-between flex-wrap gap-4'>
          ...
          <p className='font-normal text-[14px] text-white opacity-50'>
            Copywrite 2021 - 2022 Metaverse. All rights reserved.
          </p>
          <p className='flex gap-4'>
            {socials.map((social) => (
              <img key={social.name} src={social.url} className='w-[24px] h-[24px] object-contain cursor-pointer'></img>
            ))}
          </p>
        </div>
      </div>
